...
export class BedrockChatbotStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: BedrockChatbotStackProps) {
    ...
    // Lambda function
    const chatFunction = new lambdaPython.PythonFunction(this, "ChatFunction", {
      entry: path.join(__dirname, "../lambda"), // lambdaコードのディレクトリ（index.pyとrequirements.txtを置く）
      runtime: lambda.Runtime.PYTHON_3_10,
      index: "index.py", // Lambdaのエントリポイントファイル名
      handler: "lambda_handler", // 関数名
